...
export default function Hero() {
 ...
 useEffect(() => {
  ...
  const smoothUpdate = () => {
   ...
   const shouldSeek = isVideoReadyRef.current && !isSeekingRef.current && diff > 0.03;
 ...
 useGSAP(() => {
  ...
  return (
   <>
   <section id="hero" className="relative z-10 min-h-dvh w-full border border-transparent">
   <img src="/images/arrow.png" alt="arrow" className="absolute left-5 bottom-1/2" />
   <h1 className="title md:mt-32 mt-40 text-6xl md:text-[20vw] mb-20 leading-none text-center">NATI</h1>
   
   <Image
    src="/images/nav-tag.png"
    alt="left-leaf"
    width={30}
    height={30}
    className="nav-tag absolute left-0 md:top-20 xl:top-36 2xl:top-52 md:bottom-auto -bottom-20 md:w-24 lg:w-32 xl:w-40 w-20 h-auto"
   />
   {/* <Image
    src="/images/footer-tag.png"
    alt="footer-tag"
    width={256}
    height={512}
    className="footer-tag absolute right-0 md:bottom-0 xl:top-0 2xl:top-12 top-1/2 md:w-40 lg:w-52 xl:w-64 w-24 h-auto"
   /> */}
   ...
   <div className="mx-auto max-w-7xl w-full lg:bottom-20 top-auto md:top-[30vh] flex justify-between items-end px-5">
    ...
    <div className=" space-y-5 text-lg lg:max-w-2xs md:max-w-xs w-full">
     ...
     <a href="#projects" className="font-semibold opacity-80 2xl:text-start text-center hover:text-yellow-500 block">View Projects</a>
    </div>
    </div>
   </div>
   </section>
